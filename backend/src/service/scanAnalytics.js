...
exports.getMonthlyScanCount = async ({userId}) => {
    const result = await sql`
        ...
        GROUP BY gs.month_start -- Group by the generated month start date
        ORDER BY gs.month_start ASC; -- Order chronologically
    `;
    return result;
};
...
exports.getDailyScanCount = async ({userId}) => {
    const result = await sql`
        SELECT gs.hour_of_day                                                                                  AS scan_hour, -- The hour (0-23)
               -- Count model scans for the specific user AND current date, COALESCE to 0
               COALESCE(SUM(CASE WHEN p.user_id = ${userId} AND DATE
                            (qcs.scanned_at) = CURRENT_DATE AND qcs.scan_type = 'model' THEN 1 ELSE 0 END), 0) AS model,
               -- Count unit scans for the specific user AND current date, COALESCE to 0
               COALESCE(SUM(CASE WHEN p.user_id = ${userId} AND DATE
                            (qcs.scanned_at) = CURRENT_DATE AND qcs.scan_type = 'unit' THEN 1 ELSE 0 END), 0)  AS unit,
               -- Count total scans for the specific user AND current date, COALESCE to 0
               COALESCE(SUM(CASE WHEN p.user_id = ${userId} AND DATE (qcs.scanned_at) = CURRENT_DATE THEN 1 ELSE 0 END),
                        0)                                                                                     AS total
        FROM
            ...
            qr_code_scans qcs ON EXTRACT(HOUR FROM qcs.scanned_at) = gs.hour_of_day
                LEFT JOIN
            products p ON qcs.product_id = p.id
        GROUP BY gs.hour_of_day
        ORDER BY gs.hour_of_day ASC; -- Order from 0 to 23
    `;
    return result;
};
